...
echo '<div class="content">Новый топик</div>';
...
$tim = mysql_query("SELECT * FROM `forum_topic` WHERE `user` = '".$user['id']."' ORDER BY `time` DESC");
...
mysql_query("INSERT INTO `forum_topic` SET `name` = '".$name."',
`text` = '".$text."',
`user` = '".$user['id']."', 
`time` = '".time()."', 
`open` = '".$open."',
...
$uid = mysql_insert_id();
if($new == 1){
mysql_query("UPDATE `users` SET `news` = '".$uid."' WHERE `id` ");
}
