...
//if($user['ass_br'] < $ass_br){
mysql_query("UPDATE `users` SET `ass_br` = '".$ass_br."' WHERE `id` = '$user[id]' LIMIT 1");
//}
...
mysql_query("UPDATE `users` SET `ass_br` = '".($user['ass_br'] + 1)."' WHERE `id` = '$user[id]' LIMIT 1");
mysql_query("INSERT INTO `ass_br` SET `msg` = '".$msg1."', `avtor` = '".$user['id']."', `clan_id` = '".$corp['id']."', `time` = '".time()."'");
...
$max = $user['max'];
$k_post = mysql_result(mysql_query("SELECT COUNT(*) FROM `ass_br` WHERE `clan_id` = '".$user['corp']."'"),0);
$k_page = k_page($k_post,$max);
$page = page($k_page);
$start = $max*$page-$max;
$query = mysql_query("SELECT * FROM `ass_br` WHERE `clan_id` = '".$user['corp']."' ORDER BY `time` DESC LIMIT $start, $max");
...
echo' <a href="'.$HOME.'corp/otvet.php?id='.$post['id'].'"><img src="/images/reply.png" width="18" height="18"></a> ';
}
echo '<br>';
...
$gg = mysql_fetch_assoc(mysql_query("SELECT * FROM `ass_br` WHERE `id` = '".$id."'"));
if(isset($gg['id'])){
...
mysql_query("UPDATE `users` SET `ass_br` = '0' WHERE `corp` = '".$corp['id']."' ");
mysql_query('DELETE FROM `ass_br` WHERE `clan_id` = "'.$corp['id'].'" ');
