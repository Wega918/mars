...
$tim = mysql_query("SELECT * FROM `ass_br` WHERE `avtor` = '".$user['id']."' ORDER BY `time` DESC");
...
header('Location: '.$HOME.'corp/otvet.php?id='.$enot['id'].'');
...
$tspam = mysql_fetch_array(mysql_query('select * from `ass_br` where `avtor` = "'.$user['id'].'" and `msg` = "'.$msg1.'" and `time` > "'.(time()-90).'"'));
if($tspam != 0) {
header('Location: '.$HOME.'corp/otvet.php?id='.$enot['id'].'');
...
mysql_query("INSERT INTO `ass_br` SET `msg` = '[b]".$ank['login'].",[/b] ".$msg1."', `avtor` = '".$user['id']."', `clan_id` = '".$corp['id']."', `time` = '".time()."', `id_komu` = '".$ank['id']."'");
header('Location: '.$HOME.'ass_br/');
...
echo '<a class="btnl" style="margin-top:2px;" href="'.$HOME.'ass_br/"><img src="/images/header/arrow_left_white2.png" width="12" height="12"> Чат</a>';
