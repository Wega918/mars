...
$clan = mysql_fetch_assoc(mysql_query("SELECT * FROM `corp` WHERE `id`  = '".$user['corp']."'"));
...
if($user['corp_rang'] > 2 and $user['level'] == 0) {
header("Location: /corp/");
exit();
}
$corp_chat = mysql_result(mysql_query("SELECT COUNT(id) FROM `corp_chat` WHERE `clan_id` = ".$user['corp'].""),0);
//if($user['corp_chat'] < $corp_chat){
mysql_query("UPDATE `users` SET `corp_chat` = '".$corp_chat."' WHERE `id` = '$user[id]' LIMIT 1");
//}
...
$tim = mysql_query("SELECT * FROM `corp_chat` WHERE `avtor` = '".$user['id']."' ORDER BY `time` DESC");
...
$spa = 'Здравствуйте! Игрок '.$user['login'].' попытался отправить ('.$msg.') в чате кп.';
$spam = strong($spa);
...
if(empty($msg) or mb_strlen($msg) < 1) {
header('Location: ?');
$_SESSION['err'] = '<font color=red>Ошибка ввода, минимум 1 символ!</font>';
exit();
}
...
mysql_query("UPDATE `users` SET `corp_chat` = '".($user['corp_chat'] + 1)."' WHERE `id` = '$user[id]' LIMIT 1");
mysql_query("INSERT INTO `corp_chat` SET `msg` = '".$msg1."', `avtor` = '".$user['id']."', `clan_id` = '".$corp['id']."', `time` = '".time()."'");
...
echo '<center><div class="minor mb4"><font color=black>Чат доступен только Владельцу и заместителям.</font></div></center><hr>';
...
$max = $user['max'];
$k_post = mysql_result(mysql_query("SELECT COUNT(*) FROM `corp_chat` WHERE `clan_id` = '".$user['corp']."'"),0);
$k_page = k_page($k_post,$max);
$page = page($k_page);
$start = $max*$page-$max;
$query = mysql_query("SELECT * FROM `corp_chat` WHERE `clan_id` = '".$user['corp']."' ORDER BY `time` DESC LIMIT $start, $max");
...
if($user['level'] >= 1 or $user['corp_rang'] <= 2){
$del = '<a href="'.$HOME.'corp_chat/delmsg'.$post['id'].'"><b><font color=indianred>x</font></b></a>';
}else{
$del = '';
}
...
echo' <a href="'.$HOME.'corp/corp_otvet.php?id='.$post['id'].'"><img src="/images/reply.png" width="18" height="18"></a> ';
}
echo '<br>';
...
if($k_post < 1){
echo '<center><b>Пока пусто,будешь первым!</b></center>';
}

if ($k_page > 1) {
echo str(''.$HOME.'corp_chat/?',$k_page,$page); // Вывод страниц
}
...
$gg = mysql_fetch_assoc(mysql_query("SELECT * FROM `corp_chat` WHERE `id` = '".$id."'"));
if(isset($gg['id'])){
if($user['corp'] == $corp['id'] && $user['corp_rang'] <= 2 or $user['level'] >= 1){
mysql_query("DELETE FROM `corp_chat` WHERE `id` = '".$id."'");
header('Location: '.$HOME.'corp_chat/?');
...
mysql_query("UPDATE `users` SET `corp_chat` = '0' WHERE `corp` = '".$corp['id']."' ");
mysql_query('DELETE FROM `corp_chat` WHERE `clan_id` = "'.$corp['id'].'" ');
