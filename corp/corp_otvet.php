...
$enot = mysql_fetch_assoc(mysql_query("SELECT * FROM `corp_chat` WHERE `id` = '".$id."'"));
...
header('Location: '.$HOME.'corp/corp_otvet.php?id='.$enot['id'].'');
...
$spa = 'Здравствуйте! Игрок '.$user['login'].' попытался отправить ('.$_POST['msg'].') в  скрытом чате Кп.';
$spam = strong($spa);
...
$tspam = mysql_fetch_array(mysql_query('select * from `corp_chat` where `avtor` = "'.$user['id'].'" and `msg` = "'.$msg1.'" and `time` > "'.(time()-90).'"'));
if($tspam != 0) {
header('Location: '.$HOME.'corp/corp_otvet.php?id='.$enot['id'].'');
...
mysql_query("INSERT INTO `corp_chat` SET `msg` = '[b]".$ank['login'].",[/b] ".$msg1."', `avtor` = '".$user['id']."', `clan_id` = '".$corp['id']."', `time` = '".time()."', `id_komu` = '".$ank['id']."'");
header('Location: '.$HOME.'corp_chat/');
...
echo '<a class="btnl" style="margin-top:2px;" href="'.$HOME.'corp_chat/"><img src="/images/header/arrow_left_white2.png" width="12" height="12"> Чат</a>';
