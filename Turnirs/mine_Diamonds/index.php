...
Цель каждого игрока - в шахте добывать <img width="24" height="24" alt="" src="/images/Diamonds.png"> Алмазы.
<br><br></b>

<font color=red>каждый найденый алмаз в шахте - это 100% мусора
каждых три дня алмазы будут суммироваться и по итогам турнира мусор будет начисляться в союз как постоянный бонус.
...
$max = 10;
$k_post = mysql_result(mysql_query("SELECT COUNT(*) FROM `users` WHERE `turnir_Diamonds` >= '1' "),0);
$k_page = k_page($k_post,$max);
$page = page($k_page);
$start = $max*$page-$max;
$k_post = $start+1;
$corprat = mysql_query("SELECT * FROM `users` WHERE `turnir_Diamonds` >= '1' ORDER BY `turnir_Diamonds` DESC LIMIT $start,$max");
...
<span class="fr nobr"><img width="24" height="24" alt="" src="/images/Diamonds.png"><span> '.n_f($cr['turnir_Diamonds']).'</span></span>
<div class="cb"></div></div></div>';
}
$q = mysql_query("SELECT * FROM `users` WHERE `id` ORDER BY `turnir_Diamonds` DESC ");
while($post = mysql_fetch_assoc($q)){
$v1++;
if($post['id'] == $user['id']){
if($post['turnir_Diamonds'] >= 1){
