...
$topic1 = mysql_fetch_assoc(mysql_query("SELECT * FROM `corp_forum_topic` WHERE `id` = '".$topic."'"));
...
if($corp != $user['corp'] and $razdel1['dostupen'] == 1) {
...
$corp_forum_fols = mysql_fetch_array(mysql_query('SELECT * FROM `corp_forum_fols` WHERE `user` = "'.$user['id'].'" and `razdel`  = "'.$razdel.'" and `topic` = "'.$topic.'" and `corp` = "'.$corp.'" '));
if(!$corp_forum_fols){
mysql_query("INSERT INTO `corp_forum_fols` SET `user` = '".$user['id']."', `razdel` = '".$razdel."', `topic` = '".$topic."', `corp` = '".$corp."' ");
}




echo '<div class="content"><a href="'.$HOME.'corp_forum/razdel_'.$razdel.'/corp_'.$corp.'/">'.$razdel1['name'].'</a> / '.$topic1['name'].'</div>';
...
$max = $user['max'];
$ONL = mysql_result(mysql_query("SELECT COUNT(*) FROM `corp_forum_comments` WHERE `topic` = '".$topic."'"),0);
$k_post = mysql_result(mysql_query("SELECT COUNT(*) FROM `corp_forum_comments` WHERE `corp` = '".$corp."' and `topic` = '".$topic."' "),0);
$k_page = k_page($k_post,$max);
$page = page($k_page);
$start = $max*$page-$max;
$query = mysql_query("SELECT * FROM `corp_forum_comments` WHERE `corp` = '".$corp."' and `topic` = '".$topic."' ORDER BY `id` DESC LIMIT $start, $max");
while($forum_c = mysql_fetch_assoc($query)){

if($user['corp'] == $corp or $user['level'] >= 1){
if($user['corp_rang'] <= 2 or $user['level'] >= 1){
$delete = ' <a href="/corp_forum/post_del'.$forum_c['id'].'/'.$page.'/'.$corp.'/'.$razdel.'/"><font color=red>[x]</font></a>';
}else{
echo '';
}
}
...
if ($k_page > 1) {
echo str(''.$HOME.'corp_forum/razdel_'.$razdel.'/topic_'.$topic.'/corp_'.$corp.'/?',$k_page,$page); // Вывод страниц
}
...
$tim = mysql_query("SELECT * FROM `corp_forum_comments` WHERE `avtor` = '".$user['id']."' ORDER BY `time` DESC");
...
$tspam = mysql_fetch_array(mysql_query('select * from `corp_forum_comments` where `avtor` = "'.$user['id'].'" and `koment` = "'.$koment1.'" and `time` > "'.(time()-90).'"'));
if($tspam != 0) {
header('Location: ?');
...
mysql_query('DELETE FROM `corp_forum_fols` WHERE `topic` = "'.$topic.'" and `corp` = "'.$corp.'" ');
mysql_query("INSERT INTO `corp_forum_comments` SET `koment` = '".$koment1."', `avtor` = '".$user['id']."', `time` = '".time()."' , `topic` = '".$topic."', `corp` = '".$corp."' ");
header('location: ?');
...
echo '<a class="btnl" style="margin-top:2px;" href="'.$HOME.'corp_forum/seting_topic_'.$topic.'/razdel_'.$razdel.'/corp_'.$corp.'/"><img width="24" height="24" src="/images/settings2.png"> <span>Редактировать топик</span></a>';
}
}
}
...
$corp1 = abs(intval($_GET['corp']));
$razdel1 = abs(intval($_GET['razdel']));
$pagess = abs(intval($_GET['pagess']));
$forum_cc = mysql_fetch_assoc(mysql_query("SELECT * FROM `corp_forum_comments` WHERE `id`='".$id1."' "));
$forum_tt = mysql_fetch_assoc(mysql_query("SELECT * FROM `corp_forum_topic` WHERE `id` = '".$forum_cc['topic']."'"));
...
mysql_query('DELETE FROM `corp_forum_comments` WHERE `id` = '.$forum_cc['id'].' ');
$_SESSION['err'] = 'Коментарий успешно удален!';
header('Location: '.$HOME.'corp_forum/razdel_'.$razdel1.'/topic_'.$forum_tt['id'].'/corp_'.$corp1.'/?page='.$pagess.'');
exit();
break;
}
