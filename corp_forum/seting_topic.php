...
$id1 = mysql_fetch_assoc(mysql_query("SELECT * FROM `corp_forum_topic` WHERE `id` = '".$id."'"));


if($user['corp'] != $corp and $user['level'] == 0){
...
mysql_query("UPDATE `corp_forum_topic` SET `name` = '". $name ."', 
`text` = '". $text ."',  
`user_red` = '". $user['id']."',  
`time_red` = '".time()."',  
`open` = '". $open ."'  
...
mysql_query('DELETE FROM `corp_forum_topic` WHERE `id` = '.$id.' ');
...
echo '<a class="btnl" style="margin-top:2px;" href="'.$HOME.'corp_forum/razdel_'.$razdel.'/topic_'.$id.'/corp_'.$corp.'/"><img src="/images/header/arrow_left_white2.png" width="12" height="12"> Вернуться</a>';
