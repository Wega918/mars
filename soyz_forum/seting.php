...
echo '<div class="content"><a href="'.$HOME.'soyz_forum/razdel_'.$razdel.'/soyz_'.$soyz.'/">'.$razdel1['name'].'</a> / Редактирование</div>';



if (empty($user['max'])) $user['max']=10;
$max = $user['max'];
$k_post = mysql_result(mysql_query("SELECT COUNT(*) FROM `soyz_forum_razdel` WHERE `id` = '".$razdel."'  "),0);
$k_page = k_page($k_post,$max);
$page = page($k_page);
$start = $max*$page-$max;
$soyz_forum_razdel = mysql_query("SELECT * FROM `soyz_forum_razdel` WHERE `id` = '".$razdel."'  ORDER BY `id` DESC LIMIT $start, $max");
...
mysql_query("UPDATE `soyz_forum_razdel` SET `name` = '".$name."',  `dostupen` = '".$dostupen."'   WHERE `id` = '". $f['id'] ."' LIMIT 1");
header('Location: ?');
...
mysql_query('DELETE FROM `soyz_forum_razdel` WHERE `id` = '.$f['id'].' ');
