...
$id1 = mysql_fetch_assoc(mysql_query("SELECT * FROM `soyz_forum_topic` WHERE `id` = '".$id."'"));



if($user['soyz'] != $soyz and $user['level'] == 0){
if($user['soyz_rang'] > 2 and $user['level'] == 0){
if($id1['user'] != $user['id'] and $user['level'] == 0){
header('Location: /');
exit();
}
}
...
echo '<div class="content">'.$id1['name'].' / Редактирование</div>';
...
Название:<br> <input type="text" style="width: 95%;" name="name" value="'.$id1['name'].'"> <br>
Текст топика:<br /><textarea style="width: 95%;" name="text" id="message">'.$id1['text'].'</textarea><br>
...
mysql_query("UPDATE `soyz_forum_topic` SET `name` = '". $name ."', 
`text` = '". $text ."',  
`user_red` = '". $user['id']."',  
`time_red` = '".time()."',  
`open` = '". $open ."'  
WHERE `id` = '".$id1['id']."' LIMIT 1");

header('Location: ?');
$_SESSION['err'] = 'Топик  - '.$id1['name'].' -  отредактирован!';
exit();
}
...
mysql_query('DELETE FROM `soyz_forum_topic` WHERE `id` = '.$id.' ');
header('Location: '.$HOME.'soyz_forum/razdel_'.$razdel.'/soyz_'.$soyz.'/');
...
if($user['soyz_rang'] <= 3) {
echo '<a class="btnl" style="margin-top:2px;" href="?delete_'.$id.'/"><img src="/images/cross.png" width="12" height="12"> Удалить топик</a>';
}
echo '<a class="btnl" style="margin-top:2px;" href="'.$HOME.'soyz_forum/razdel_'.$razdel.'/topic_'.$id.'/soyz_'.$soyz.'/"><img src="/images/header/arrow_left_white2.png" width="12" height="12"> Вернуться</a>';
