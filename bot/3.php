...
mysql_query("UPDATE `bot` SET `bot` = '3', `time` = '".(time()+86400)."' WHERE `id` = '".$bot['id']."' ");
...
mysql_query("UPDATE `bot` SET `bot` = '3', `time` = '".(time()+(86400*5))."' WHERE `id` = '".$bot['id']."' ");
...
mysql_query("UPDATE `bot` SET `bot` = '3', `time` = '".(time()+(86400*15))."' WHERE `id` = '".$bot['id']."' ");
...
mysql_query("UPDATE `bot` SET `bot` = '3', `time` = '".(time()+(86400*30))."' WHERE `id` = '".$bot['id']."' ");
...
echo '<center>
<div class="header-2019 cntr"><font color=#e9e9e9>Рембо</font></div>

<img src="/images/bots/image3.png" alt="" style="width:50%; margin:5px;border-radius: 8px;">

<div class="content">
<a class="fl" href="/bot/2/"><img width="100%" src="/images/index/left_orange.png" alt="<" title="<"></a>
<a class="fr" href="/bot/1/"><img width="100%" src="/images/index/right_orange.png" alt=">" title=">"></a>
<div class="center">';
if($bot['time'] < time()){
...
}elseif($bot['time'] > time() and $bot['bot'] == 3){
...
<font size=2% color=green>Рембо - автоматически собирает урожай с ваших грядок.<br>
Ускоряет дозревания урожая на 50%<br>
Автоматически пополняет энергию при ее отсутствии.<br>
Повышет прибыль листиков в 2 раза.</font>
</span></li></ul></div>
