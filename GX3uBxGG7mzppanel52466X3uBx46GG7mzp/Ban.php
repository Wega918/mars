...
$ban1 = mysql_fetch_array(mysql_query('SELECT * FROM `ban` WHERE `user` = "'.$id.'"'));
...
$tex = 'Здравствуйте! Ваш аккаунт был разблокироан игроком '.$user['login'].'. Комментарий: '.$komm.'. ';
$text = strong($tex);
...
$txt = '<font color=black>Разблокирован:</font> '.nick($id).'<br>
<font size=2>Комментарий: <b>'.$komm.'</b></font>';
mysql_query("INSERT INTO `narushenija` SET `user` = '".$id."', `text` = '".$txt."', `time` = '".time()."', `time_end` = '".(time() + $time)."', `kto` = '".$user['id']."' ");

$_SESSION['err']='Игрок '.nick($id).' разблокирован.';
...
if(empty($prich)) {
header('Location: ?');
$_SESSION['err'] = '<font color=red>Вы не ввели причину блокировки!</font>';
exit();
}
$time_sett = ''.$time.'ч.';
$txt = '<font color=red>Заблокирован:</font> '.nick($id).'<br>
<font size=2>Причина: <b>'.$prich.'</b><br>
Заблокирован на: '.$time_sett.'<br>
Заблокирован до: '.vremja(time() + ($time*3600)).'</font>';
mysql_query("UPDATE `users` SET `colors`='D3D3D3' WHERE `id` = '".$id."' limit 1");
mysql_query("INSERT INTO `ban` SET `user` = '".$id."', `prich` = '".$prich."', `time` = '".time()."', `time_end` = '".(time() + ($time*3600))."', `kto` = '".$user['id']."' ");
mysql_query("INSERT INTO `narushenija` SET `user` = '".$id."', `text` = '".$txt."', `time` = '".time()."', `time_end` = '".(time() + ($time*3600))."', `kto` = '".$user['id']."' ");
$_SESSION['err'] = 'Игрок '.nick($id).' заблокирован до '.vremja(time() + ($time*3600)).'.';
header ('Location: '.$HOME.'igrok_'.$id.'/');
exit();
}
