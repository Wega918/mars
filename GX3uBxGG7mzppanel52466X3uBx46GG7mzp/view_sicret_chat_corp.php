<?php
$title = 'Просмотр скрытого чата Корпорации';
...
$max = $user['max'];
$k_post = mysql_result(mysql_query("SELECT COUNT(*) FROM `corp_chat` WHERE `clan_id` = '".$ank['id']."'"),0);
$k_page = k_page($k_post,$max);
$page = page($k_page);
$start = $max*$page-$max;
$query = mysql_query("SELECT * FROM `corp_chat` WHERE `clan_id` = '".$ank['id']."' ORDER BY `time` DESC LIMIT $start, $max");
...
mysql_query("DELETE FROM `corp_chat` WHERE `id` = '".$post['id']."'");
header('Location: ?');
...
if ($k_page > 1) {
echo str(''.$HOME.'view_sicret_chat_corp/'.$id.'/?',$k_page,$page); // Вывод страниц
}
