...
$ank = mysql_fetch_assoc(mysql_query("SELECT * FROM `soyz` WHERE `id` = '".$id."'"));


if($ank == 0){
$_SESSION['err']='Такого Союза не существует!';
header ('Location: /');
exit();
}
...
$max = $user['max'];
$k_post = mysql_result(mysql_query("SELECT COUNT(*) FROM `soyz_ass` WHERE `clan_id` = '".$ank['id']."'"),0);
$k_page = k_page($k_post,$max);
$page = page($k_page);
$start = $max*$page-$max;
$query = mysql_query("SELECT * FROM `soyz_ass` WHERE `clan_id` = '".$ank['id']."' ORDER BY `time` DESC LIMIT $start, $max");
...
mysql_query("DELETE FROM `soyz_ass` WHERE `id` = '".$post['id']."'");
header('Location: ?');
...
if ($k_page > 1) {
echo str(''.$HOME.'view_chat_soyz/'.$id.'/?',$k_page,$page); // Вывод страниц
}



echo '<a class="btnl mt4" href="'.$HOME.'soyz/'.$id.'/"><img src="/images/header/arrow_left_white2.png" width="12" height="12" alt="" title=""> Вернуться</a>';
